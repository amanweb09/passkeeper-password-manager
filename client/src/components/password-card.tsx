...
const PasswordCard: React.FC<IPropTypes> = ({ credential, setCurrentCredential, selectedCredential }) => {
    const icon = credential.domain.split(".")[1][0] || ""
    
    return (
        <div 
        onClick={() => setCurrentCredential(credential)}
        className={`${selectedCredential?.uid === credential.uid ? "bg-sky-200" : "bg-white"} w-full py-6 px-2 rounded-md flex items-center hover:bg-neutral-50 cursor-pointer`}>
